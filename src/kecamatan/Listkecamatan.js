...
export default function Listkecamatan({route, navigation}) {
  ...
  return (
    <NativeBaseProvider>
      ...
      <Header tit={'Infrastruktur Kecamatan'} nv={navigation} conf={conf} />
      ...
      <Box bg={'gray.100'} flex={1}>
        ...
        <FlatList
          ...
          renderItem={({item, index}) => (
            <Pressable
              onPress={() => navigation.navigate('Detailkecamatan')}
              disabled={!item.act}
              key={index}>
              <Box
                mx={4}
